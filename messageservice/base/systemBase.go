...
func initDB(dbConfig databaseConfig){
	...
	DBEngine.SetMaxOpenConns(200)	//设置最大打开的连接数，默认值为0表示不限制
	...
	//同步表结构
	err = DBEngine.Sync2(new(models.Message), new(models.CustomerServiceTemplateMessage), new(models.TemplateMessageData), new(models.AppMessage))
	if err != nil {
